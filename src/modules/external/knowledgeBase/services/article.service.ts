...
export class ArticleService implements IArticleService {
   ...
   *  restoreArticles. */
  async archiveArticles(versionIds: string[]): Promise<void> {
    const versions = await this.articleRepository.findByVersionIds(versionIds);
    if (versions.length === 0) throw new BusinessLogicError('No versions found.');
  ...
  async restoreArticles(versionIds: string[]): Promise<void> {
    const versions = await this.articleRepository.findByVersionIds(versionIds);
    if (versions.length === 0) throw new BusinessLogicError('No versions found.');

    const versionByArticle = new Map<string, string>();
    for (const v of versions) {
    ...
    for (const [articleId, anyVersionId] of versionByArticle) {
      ...
      if (all.some((v) => v.article_status !== 'archived')) {
        throw new BusinessLogicError(
          `Article "${all[0]?.article_name ?? articleId}" is not archived.`,
        );
      }
  ...
  async createManagedArticle(
    ...
    if (data.synopsis) {
      ...
      await this.articleRepository.updateClientCopy(
        created.article_id,
