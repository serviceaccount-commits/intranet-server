...
export class ArticleSearchService {
  ...
  private async vectorSearch(
    ...
    const best: Array<{ version_id: string; chunk_id: string; preview: string; similarity: number }> = [];
  ...
  private async clientTextSearch(
    query: string,
    topicIds?: string[],
  ): Promise<Array<{ version_id: string }>> {
    const words = query
      .split(/\s+/)
      .map((w) => w.trim())
      .filter((w) => w.length >= 3)
    ...
    const scored: Array<{ version_id: string; score: number }> = [];
    for (const d of docs) {
      ...
      for (const re of patterns) {
        ...
        else if (re.test(synopsis)) score += 2;
      ...
      if (allHit && score > 0) scored.push({ version_id: copy._id.toString(), score });
    }
...
export function buildPreview(chunk: string, query: string): string {
  ...
  if (bestIdx < 0) return `${text.slice(0, PREVIEW_LENGTH).trimEnd()}…`;
  ...
  if (out.length > PREVIEW_LENGTH) out = `${out.slice(0, PREVIEW_LENGTH).trimEnd()}…`;
