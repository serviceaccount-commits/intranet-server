...
const ADMIN_API_KEY = appConfig_1.default.internalAdminApiKey;
const verifyAdminApiKey = async (req, res, next) => {
    if (!ADMIN_API_KEY) {
        logger_1.logger.error('INTERNAL_ADMIN_API_KEY is not set in environment variables.');
    ...
    const isVerified = crypto.timingSafeEqual(Buffer.from(clientApiKey), Buffer.from(ADMIN_API_KEY));
    if (isVerified) {
        next();
    }
    else {
...
//# sourceMappingURL=adminApiKey.middleware.js.map