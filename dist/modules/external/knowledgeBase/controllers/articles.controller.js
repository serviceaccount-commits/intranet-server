...
let ArticleController = class ArticleController {
    ...
    async generateAISynopsis(req, res) {
        ...
        try {
            ...
            const quotaExhausted = status === 429 || /\b429\b|RESOURCE_EXHAUSTED|quota|Too Many Requests/i.test(message);
            if (quotaExhausted) {
                res.status(429).json({
                    error: {
                        code: 'AI_QUOTA_EXHAUSTED',
                        message: 'The AI synopsis quota is exhausted for now (daily limit reached). Try again later or write the synopsis manually.',
                    },
                });
                return;
            }
            res.status(502).json({
                error: {
                    ...
                    message: 'The AI synopsis could not be generated right now. Try again in a moment or write it manually.',
                },
            });
        }
    }
